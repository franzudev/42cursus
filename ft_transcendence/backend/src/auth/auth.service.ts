...
export class AuthService {
    ...
    generateQrCode(username: string, secret_code: string): any {
        return this.httpService.get(
            `https://www.authenticatorapi.com/pair.aspx?AppName=Transcendence&AppInfo=${username}&SecretCode=${secret_code}`
        );
    }

    verifyGCode(code: string, secret_code: string): any {
        return this.httpService.get(
            `https://www.authenticatorapi.com/Validate.aspx?Pin=${code}&SecretCode=${secret_code}`
        );
    }
